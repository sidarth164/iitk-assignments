...
NachOSThread::NachOSThread(char* threadName, int p)
{
    int i;
    name = threadName;
    stackTop = NULL;
...
NachOSThread::~NachOSThread()
{
    ...
    if(pid!=0){ 
    long long unsigned time=stats->totalTicks-threadStartTime;
    DEBUG('z',"thread[%d] squared thread completion time = %llu\n",pid,stats->squaredSumThreadCompletionTime);
    DEBUG('z',"time=%llu, time2=%llu\n",time,time*time);
    stats->sumThreadCompletionTime+=time;
    stats->squaredSumThreadCompletionTime+=(time*time);
    if(time>stats->maxThreadCompletionTime)
        stats->maxThreadCompletionTime=time;
    if(time<stats->minThreadCompletionTime)
        stats->minThreadCompletionTime=time;
    stats->numberOfThreads+=1;
    }
...
NachOSThread::Exit (bool terminateSim, int exitcode)
{
    ...
    if(pid!=0 && scheduler->algo==2){
        int lerror = currentThread->estmPrevBurstTime-currentThread->truePrevBurstTime;
        stats->error+=(lerror>0?lerror:-lerror);
    }
    ...
    while ((nextThread = scheduler->SelectNextReadyThread()) == NULL) {
        if (terminateSim) {
            if(pid!=0){
                long long unsigned time=stats->totalTicks-threadStartTime;
                DEBUG('z',"thread[%d] thread completion time = %llu\n",pid,stats->squaredSumThreadCompletionTime);
                stats->sumThreadCompletionTime+=time;
                stats->squaredSumThreadCompletionTime+=(time*time);
                if(time>stats->maxThreadCompletionTime)
                    stats->maxThreadCompletionTime=time;
...
NachOSThread::YieldCPU ()
{
    ...
    for(int i=0;i<MAX_THREAD_COUNT;i++){
        if(threadArray[i]!=NULL)
           DEBUG('t',"Thread[%d] base priority=%d priority=%d\n",i,basePriority[i],Priority[i]); 
    }

    DEBUG('t', "Yielding thread \"%s\" with pid %d with priority:%d\n", getName(), pid, Priority[pid]);
    
    nextThread = scheduler->SelectNextReadyThread();
    if (nextThread != NULL) {
        if(scheduler->algo>=7 && Priority[currentThread->GetPID()]<Priority[nextThread->GetPID()]){
...
NachOSThread::setStatus(ThreadStatus st){
    if(st==JUST_CREATED){
        status=st;
        tau=0;
        threadStartTime=stats->totalTicks;
    }
    
    if(status == RUNNING && st!= RUNNING && stats->totalTicks!=startBurstTime && scheduler->algo>6){
        CPUUsage[pid]+=stats->totalTicks-startBurstTime;
        DEBUG('e',"Updating CPU Usage of thread with pid:%d by %d\n",pid,CPUUsage[pid]);
        for(int i=0;i<MAX_THREAD_COUNT;i++){
            CPUUsage[i]/=2;
            Priority[i]=basePriority[i]+CPUUsage[i]/2;
        }
    }

    if(status == RUNNING){
        if(st== READY){
            DEBUG('2',"Running->Ready\n");
            truePrevBurstTime=stats->totalTicks-startBurstTime;
            startWaitTime=stats->totalTicks;
            if(pid!=0 && truePrevBurstTime!=0){
                stats->totalBurstTime+=truePrevBurstTime;
                stats->totalBurstNumber+=1;
                stats->maxBurstTime=(stats->maxBurstTime>truePrevBurstTime?stats->maxBurstTime:truePrevBurstTime);
                stats->minBurstTime=(stats->minBurstTime<truePrevBurstTime?stats->minBurstTime:truePrevBurstTime);
            }
        }
        else if(st == BLOCKED){
            truePrevBurstTime=stats->totalTicks-startBurstTime;
            DEBUG('2',"[%d][Running->Blocked]Prev Burst Time :%d\n",pid,truePrevBurstTime);
            tau=0;
            startIdleTime=stats->totalTicks;
	    //DEBUG('e',"\nStartBurstTime:%d\ntruePrevBurstTime:%d\nTotalBurstTime:%d\n",startBurstTime,truePrevBurstTime,stats->totalBurstTime);
            if(pid!=0 && truePrevBurstTime!=0){
                stats->totalBurstTime+=truePrevBurstTime;
                stats->totalBurstNumber+=1;
                stats->maxBurstTime=(stats->maxBurstTime>truePrevBurstTime?stats->maxBurstTime:truePrevBurstTime);
    ...
    if(status==READY && st==RUNNING){
       startBurstTime=stats->totalTicks; 
       if(pid!=0){
       	stats->totalWaitTime+=stats->totalTicks-startWaitTime;
	stats->numberWaits+=1;
       }
    }
    if(status==BLOCKED && st==READY){
        startWaitTime=stats->totalTicks;
        DEBUG('2',"[%d][Blocked->Running]Prev Burst Time :%d\n",pid,truePrevBurstTime);
        if(pid!=0)
            stats->totalIOTime+=stats->totalTicks-startIdleTime;
    }
    status=st;
}
